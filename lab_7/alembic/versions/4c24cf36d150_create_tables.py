...
Revision ID: 4c24cf36d150
Revises: 
Create Date: 2025-02-16 23:49:53.088053
...
def upgrade():
    op.create_table(
        ...
        sa.CheckConstraint('debut_year >= 1860 AND debut_year <= EXTRACT(YEAR FROM CURRENT_DATE)', name='check_artist_debut_year'),
        sa.CheckConstraint('LENGTH(BTRIM(name)) > 0', name='check_name_not_empty_string'),
        sa.CheckConstraint('BTRIM(name) = name', name='check_name_without_spaces'),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint('release_year >= 1860 AND release_year <= EXTRACT(YEAR FROM CURRENT_DATE)', name='check_song_release_year'),
